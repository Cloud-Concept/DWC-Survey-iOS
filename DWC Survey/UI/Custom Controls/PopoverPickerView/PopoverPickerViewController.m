...
- (id)initWithPickerSourceJsonArray:(NSArray*)source defaultSelectIndex:(NSInteger)index
{
	self = [super initWithNibName:@"PopoverPickerViewController" bundle:nil];
    if (self) {
        self.pickerSourceJsonArray = source;
...
- (IBAction)doneClicked:(id)sender {
	if([self.pickerSourceJsonArray count] > 0)
	{
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
	...
	if([self.pickerSourceJsonArray count] > 0)
		return [self.pickerSourceJsonArray count];
	else
		return 1;
}
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component
{
	if([self.pickerSourceJsonArray count] > 0)
	{
		NSDictionary *obj = [self.pickerSourceJsonArray objectAtIndex:row];
		return [obj valueForKeyPath:@"Name"];
	}
	else
		return @"No Surveys";
}
